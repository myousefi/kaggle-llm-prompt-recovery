...
dataset = dataset.filter(lambda entry: len(entry["text"]) < 500)
...
gemini_generated_prompts = pd.read_csv(
...
while True:
    for original_text in iter(dataset):
        ...
        chat_template = f"""<start_of_turn>user
            {rewrite_prompt}:
            {original_text["text"]}<end_of_turn>
            <start_of_turn>model
            """
        ...
        outputs = model.generate(
            **inputs,
            max_new_tokens=2 * len(original_text["text"].split()),
            do_sample=False,
        )
